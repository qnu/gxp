...
def signal_handler(signum, frame):
    stdout('Signal handler called with signal %s' % signum)
...
def parse_args():
    ...
    parser.add_argument('--timeout', dest='timeout', type=int, default=5,
        help='request timeout in second')
    parser.add_argument('--overwrite', dest='overwrite', action="store_true",
        default=False, help='overwrite objects if exist')
...
class S3IOBenchmark:
    ...
    def create_keys(self, objects):
        ...
        for o in objects:
            ...
            key.key = "%s-%s-%d.%s.%s" % (
                uuid.uuid3(uuid.NAMESPACE_DNS, str(99999999 - o).encode()).hex,
    ...
    def write(self, objects):
        ...
        for k in self.create_keys(objects):
            ...
            try:
                ...
                b = k.set_contents_from_string(contents, headers=None,
    ...
    def read(self, objects):
        ...
        with open(os.devnull, 'w') as f:
            ...
            for k in self.create_keys(objects):
                ...
                self.debug("read %d bytes from %s in %f seconds" %
